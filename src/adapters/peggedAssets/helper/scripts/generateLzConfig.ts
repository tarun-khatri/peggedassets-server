...
async function generateForAsset(
  ...
  for (const [lzKey, info] of Object.entries<any>(metadata)) {
    ...
    for (const [address, token] of Object.entries<any>(tokens)) {
      ...
      if (typeof token.decimals !== "number") {
        console.warn(
          `[${peggedAsset.gecko_id}] token ${lzKey} ${address} (${symbol}) is missing decimals, skipping`
        );
      } else
        hits.push({
          llamaKey,
  ...
  lines.push(
    `// Asset: ${peggedAsset.name} (${peggedAsset.gecko_id})   Match: ${sourceAddress ? `address=${sourceAddress}` : `symbols=${[...normalizedSymbols].join("|")}`
    }   Source (auto-detected): ${source}   Filter: ${explicitOftTypes
